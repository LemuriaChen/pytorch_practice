...
# loss / w2 = loos / y2 * y2 / w2
# loss / w1 = loos / y2 * y2 / y1 *  y1 / w1

for t in range(500):
    # Forward pass: compute predicted y
    h = x.dot(w1)
    h_relu = np.maximum(h, 0)     # y1
    y_pred = h_relu.dot(w2)       # y2

    # Compute and print loss
    loss = np.square(y_pred - y).sum()    # loss
    print(t, loss)
...
# optimize the model using the Adam algorithm provided by the optim package:
# N is batch size; D_in is input dimension;
# H is hidden dimension; D_out is output dimension.
N, D_in, H, D_out = 64, 1000, 100, 10
